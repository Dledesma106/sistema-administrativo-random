...
export class AfipService {
   ...
   */
  static async getBillAFIP({ tipoCbte, ptoVta, nroCbte }: { tipoCbte: number, ptoVta: number, nroCbte: number }) {
    try {
      const result = await afip.ElectronicBilling.getVoucherInfo(nroCbte, tipoCbte, ptoVta);
      return result;
    } catch (error) {
   ...
   */
  static async getLastBillNumberAFIP({ tipoCbte, ptoVta }: { tipoCbte: number, ptoVta: number }) {
    try {
