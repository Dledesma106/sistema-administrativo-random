...
export async function emitirFacturaElectronica(billId: string) {
    ...
    const totalAmmount = bill.details.reduce(
        (acc: number, d: any) => acc + d.unitPrice * d.quantity,
        0,
    );
    ...
    const invoiceData: InvoiceTemplateData = {
        ...
        total: bill.details.reduce(
            (acc: number, d: any) => acc + d.unitPrice * d.quantity,
            0,
        ),
        cae: caeData?.code || '',
        caeDueDate: caeData?.expirationDate
            ? new Date(caeData.expirationDate).toLocaleDateString('es-AR')
            : '',
        qrDataUrl: qrUrl,
        footerText: '',
    };
