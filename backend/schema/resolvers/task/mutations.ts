...
export const downloadTaskPhotos = async (args: {
    ...
    try {
        ...
        if (tasks.length === 0) {
            throw new Error(
                `No se encontraron tareas finalizadas con imágenes para la empresa seleccionada en el período del ${format(startDate, 'dd/MM/yyyy')} al ${format(endDate, 'dd/MM/yyyy')}`,
            );
        }
        ...
        for (const task of tasks) {
            for (let i = 0; i < task.images.length; i++) {
                ...
                try {
                    ...
                    throw new Error(
                        `Error al procesar las imágenes de la tarea ${task.taskNumber}: ${error instanceof Error ? error.message : 'Error desconocido'}`,
                    );
                }
            }
        }
...
builder.mutationFields((t) => ({
    ...
    downloadTaskPhotos: t.string({
        ...
        resolve: async (root, args, _context, _info) => {
            try {
                const result = await downloadTaskPhotos({
                ...
                // Asegurarnos de que el error se propague correctamente
